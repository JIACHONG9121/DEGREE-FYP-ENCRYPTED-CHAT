...
if (!preg_match("/^[a-zA-Z\s-]+$/", $fname) || !preg_match("/^[a-zA-Z\s-]+$/", $lname)) {
    ...
    echo '<script>window.location.href="../addLU.php";</script>';
    exit;
}
...
$error_message = "";
$prefix = 'LP';
$countQuery = "SELECT COUNT(*) as userCount FROM lecturer";
                    ...
                    if (in_array($img_ext, $extensions) === true) {
                        ...
                        if (in_array($img_type, $types) === true) {
                            ...
                            if (move_uploaded_file($tmp_name, "images/" . $new_img_name)) {
                                ...
                                $insert_query = mysqli_query($conn, "INSERT INTO lecturer (unique_id, fname, lname, address, phonenumber, gender, email, password, img, status)
                                ...
                                if ($insert_query && $insert_query2) {
                                    $select_sql2 = mysqli_query($conn, "SELECT * FROM lecturer WHERE email = '{$email}'");
