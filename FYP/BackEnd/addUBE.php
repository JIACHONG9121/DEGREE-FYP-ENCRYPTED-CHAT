...
if (!preg_match("/^[a-zA-Z\s-]+$/", $fname) || !preg_match("/^[a-zA-Z\s-]+$/", $lname)) {
    ...
    echo '<script>window.location.href="../addSU.php";</script>';
    exit;
}
...
$Icode = mysqli_real_escape_string($conn, trim($_POST['Intakecode']));
...
$error_message = "";
$prefix = 'TP';
$countQuery = "SELECT COUNT(*) as userCount FROM student";
...
if (!empty($fname) && !empty($lname) && !empty($Saddress) && !empty($phonenumber) && !empty($gender) && !empty($Icode) && !empty($password)) {
                    ...
                    if (in_array($img_ext, $extensions) === true) {
                        ...
                        if (in_array($img_type, $types) === true) {
                            ...
                            if (move_uploaded_file($tmp_name, "images/" . $new_img_name)) {
                                ...
                                $insert_query = mysqli_query($conn, "INSERT INTO student (unique_id, fname, lname, intakecode, address, phonenumber, gender, email, password, img, status)
                                VALUES ('{$generatedUserID}', '{$fname}','{$lname}', '{$Icode}', '{$Saddress}', '{$phonenumber}', '{$gender}', '{$email}', '{$encrypt_pass}', '{$new_img_name}', '{$status}')");
                                ...
                                if ($insert_query && $insert_query2) {
                                    $select_sql2 = mysqli_query($conn, "SELECT * FROM student WHERE email = '{$email}'");
